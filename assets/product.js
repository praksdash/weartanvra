...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 <div class="offer-box"><strong>${esc(TANVRA_CONFIG.prepaidCoupon.code)}</strong><span>Save an extra ${TanvraStore.money(TANVRA_CONFIG.prepaidCoupon.discount)} on prepaid orders.</span></div>
 ...
 const loadMainCandidate=(start=0)=>{
   let i=start;
   const tryNext=()=>{
     if(i>=imageCandidates.length){main.removeAttribute('src');return}
     const src=imageCandidates[i++];
     const probe=new Image();
     probe.onload=()=>{main.src=src;main.dataset.candidate=String(i-1)};
 ...
 main.onerror=()=>{
   const next=(Number(main.dataset.candidate||-1)+1);
   loadMainCandidate(next);
 };
 loadMainCandidate(0);
 root.querySelectorAll('[data-thumb]').forEach(b=>b.onclick=()=>{active=+b.dataset.thumb;const im=b.querySelector('img');if(im&&im.naturalWidth)main.src=im.currentSrc||im.src;else main.src=p.images[active];root.querySelectorAll('[data-thumb]').forEach(x=>x.classList.toggle('active',x===b))});
 root.querySelectorAll('[data-color]').forEach(b=>b.onclick=()=>{color=b.dataset.color;root.querySelector('[data-color-name]').textContent=color;const probe=new Image();probe.onload=()=>main.src=b.dataset.image;probe.onerror=()=>loadMainCandidate(0);probe.src=b.dataset.image;root.querySelectorAll('[data-color]').forEach(x=>x.classList.toggle('active',x===b))});
